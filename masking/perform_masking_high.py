...
dev_laughter = numpy.concatenate((numpy.matrix(0),numpy.matrix(dev_probs[:,1]),numpy.matrix(0)),axis=1)
dev_filler = numpy.concatenate((numpy.matrix(0),numpy.matrix(dev_probs[:,2]),numpy.matrix(0)),axis=1)
...
numpy.savetxt('temp0',dev_mask_laugh.T,'%f')
...
for id_win_num in range(0,win_start_use.shape[1]):
	dev_laughter[0,win_start_use[0,id_win_num]:win_end_use[0,id_win_num]] = 1
...
# setting values to ones 
for id_win_num in range(0,win_start_use.shape[1]):
        dev_filler[0,win_start_use[0,id_win_num]:win_end_use[0,id_win_num]] = 1
